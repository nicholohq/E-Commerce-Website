...
if (!isLoggedIn()) { header('Location: ' . url('/user/login.php')); exit; }
...
if ($order_id <= 0) { header('Location: ' . url('/index.php')); exit; }

$order_stmt = $pdo->prepare("SELECT * FROM orders WHERE order_id = ? AND user_id = ?");
$order_stmt->execute([$order_id, $user_id]);
$order = $order_stmt->fetch();

if (!$order) { setFlashMessage('error', 'Order not found.'); header('Location: ' . url('/user/orders.php')); exit; }
